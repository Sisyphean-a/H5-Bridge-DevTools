...
`chrome.storage.local` 中的 `h5BridgeDevTools:v2` 是共享状态来源（`src/shared/constants.ts`、`src/shared/storage.ts`）。状态先按 origin，再按活动 bridge profile 分桶；每个 origin 同时保存方案定义和各方案的发送条目、日志、设置。内置方案兼容既有数据，外部规则包可动态新增方案（`src/shared/bridgeTypes.ts`）。content 和 panel 都监听该存储键刷新已打开实例，panel 的未保存草稿仍保留并提示远端更新（`src/content/runtime.ts`、`src/panel/runtimeBridge.ts`、`src/panel/helpers.ts`）。
